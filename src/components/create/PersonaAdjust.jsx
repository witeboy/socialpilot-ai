...
export default function PersonaAdjust({ userPersona, onComplete }) {
  ...
  return (
    <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-md p-5 sm:p-7 space-y-6">
      ...
      <div>
        <Label className="text-slate-700 dark:text-slate-300 font-semibold">Professional Tone</Label>
        <Select value={tone} onValueChange={setTone}>
          <SelectTrigger className="h-11 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-4 text-slate-900 dark:text-white mt-2 focus:outline-none focus:ring-4 focus:ring-teal-100 dark:focus:ring-teal-900">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      {userPersona?.persona_profile && (
        <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
          <div>
            <p className="text-xs text-slate-600 dark:text-slate-400 mb-1 font-semibold">Writing Style</p>
            <p className="text-sm text-slate-900 dark:text-white">{userPersona.persona_profile.writing_style || 'Not set'}</p>
          </div>
          
          {userPersona.persona_profile.expertise_areas?.length > 0 && (
            <div>
              <p className="text-xs text-slate-600 dark:text-slate-400 mb-1 font-semibold">Expertise Areas</p>

    ...
    <div className="space-y-4">
      <Card className="bg-white border border-slate-200 rounded-xl shadow-md p-5 sm:p-7 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Automation Settings</h3>
            <p className="text-sm text-slate-600">Configure posting automation</p>
          </div>
        </div>

        <div>
          <Label className="text-slate-700 mb-3 block font-semibold">Automation Mode</Label>
          <div className="space-y-3">
            <div 
              className={`p-[18px] rounded-xl border-2 cursor-pointer transition-all ${
                isAuto ? 'border-[#0FB5BA] bg-[#DDF7F8]' : 'border-slate-200 bg-white hover:border-slate-300'
              }`}
              onClick={() => setIsAuto(true)}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Zap className={`w-6 h-6 ${isAuto ? 'text-[#0FB5BA]' : 'text-slate-400'}`} />
                  <div>
                    <p className="font-bold text-slate-900">Auto Mode</p>
                    <p className="text-xs text-slate-600">System posts automatically</p>
                  </div>
                </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Clock className={`w-6 h-6 ${!isAuto ? 'text-[#0FB5BA]' : 'text-slate-400'}`} />
                  <div>
                    <p className="font-bold text-slate-900">Semi-Auto Mode</p>
                    <p className="text-xs text-slate-600">Review before posting</p>
                  </div>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  !isAuto ? 'border-[#0FB5BA]' : 'border-slate-300'
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label className="text-slate-700 text-sm mb-2 block font-semibold">Generation Time</Label>
            <input
              type="time"
              value={genTime}
              onChange={(e) => setGenTime(e.target.value)}
              className="w-full h-11 rounded-lg bg-white border border-slate-200 px-4 text-slate-900 focus:outline-none focus:ring-4 focus:ring-teal-100"
            />
          </div>

          <div>
            <Label className="text-slate-700 text-sm mb-2 block font-semibold">Posting Time</Label>
            <input
              type="time"
              value={postTime}

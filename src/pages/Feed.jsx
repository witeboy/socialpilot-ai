...
export default function Feed() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 pb-20">
      <div className="max-w-2xl mx-auto space-y-3">
        {/* Header */}
        <div className="text-center pt-2 pb-1">
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Content Feed</h1>
          <p className="text-[10px] sm:text-xs text-slate-600 mt-0.5">Manage your content pipeline</p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-1.5">
          <Card 
            ...
            className={`bg-white border rounded-lg p-2 text-center shadow-sm cursor-pointer transition-all hover:shadow-md ${
              activeTab === 'queue' ? 'border-[#0FB5BA] bg-[#DDF7F8]' : 'border-slate-200'
            }`}
          >
            <Clock className="w-4 h-4 mx-auto mb-0.5 text-amber-500" />
            <p className="text-base font-bold text-slate-900">{pendingDrafts.length}</p>
            <p className="text-[9px] text-slate-600">In Queue</p>
          </Card>
          <Card 
          ...
          >
            <CheckCircle className="w-4 h-4 mx-auto mb-0.5 text-green-500" />
            <p className="text-base font-bold text-slate-900">{approvedDrafts.length}</p>
            <p className="text-[9px] text-slate-600">Approved</p>
          </Card>
          <Card 
          ...
          >
            <Calendar className="w-4 h-4 mx-auto mb-0.5 text-blue-500" />
            <p className="text-base font-bold text-slate-900">{scheduledPosts.length}</p>
            <p className="text-[9px] text-slate-600">Scheduled</p>
          </Card>
          <Card 
          ...
          >
            <Send className="w-4 h-4 mx-auto mb-0.5 text-[#0FB5BA]" />
            <p className="text-base font-bold text-slate-900">{postedContent.length}</p>
            <p className="text-[9px] text-slate-600">Posted</p>
          </Card>
        </div>

        {/* Tabs for Content Sections */}
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4 bg-slate-100 border border-slate-200 rounded-full p-0.5 text-[10px]">
            <TabsTrigger value="queue" className="data-[state=active]:bg-[#0FB5BA] data-[state=active]:text-white text-slate-700 rounded-full h-8 font-semibold">
              Queue
            </TabsTrigger>
            <TabsTrigger value="approved" className="data-[state=active]:bg-[#0FB5BA] data-[state=active]:text-white text-slate-700 rounded-full h-8 font-semibold">
              Approved
            </TabsTrigger>
            <TabsTrigger value="scheduled" className="data-[state=active]:bg-[#0FB5BA] data-[state=active]:text-white text-slate-700 rounded-full h-8 font-semibold">
              Scheduled
            </TabsTrigger>
            <TabsTrigger value="posted" className="data-[state=active]:bg-[#0FB5BA] data-[state=active]:text-white text-slate-700 rounded-full h-8 font-semibold">
              Posted
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="queue" className="space-y-3 mt-3">
            {loadingPending ? (
              ...
              <Card className="bg-white border border-slate-200 rounded-lg p-6 text-center shadow-sm">
                <Clock className="w-10 h-10 mx-auto mb-2 text-slate-300" />
                <h3 className="text-base font-bold text-slate-900 mb-1">No Pending Posts</h3>
                <p className="text-xs text-slate-600">All caught up! Create new content to get started.</p>
              </Card>
            ) : (
              <>
                {needsManualApproval && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 flex items-center gap-1.5">
                    <span className="text-[10px] text-amber-700 font-medium">
                      ⚠️ First 3 approvals: {userPersona?.approved_posts_count || 0}/3
                    </span>
                  </div>
                )}
                <p className="text-center text-xs text-slate-600 mb-1">Swipe right to approve, left to reject</p>
                ...
                <div className="text-center mt-2">
                  <div className="inline-block bg-white shadow-sm px-3 py-1 rounded-full border border-slate-200">
          ...
          <TabsContent value="approved" className="space-y-2 mt-3">
            {approvedDrafts.length === 0 ? (
              <Card className="bg-white border border-slate-200 rounded-lg p-6 text-center shadow-sm">
                ...
                <h3 className="text-base font-bold text-slate-900 mb-1">No Approved Posts</h3>
                <p className="text-xs text-slate-600">Approve posts from the queue to see them here.</p>
              </Card>
            ) : (
          ...
          <TabsContent value="scheduled" className="space-y-2 mt-3">
            {scheduledPosts.length === 0 ? (
              <Card className="bg-white border border-slate-200 rounded-lg p-6 text-center shadow-sm">
                ...
                <h3 className="text-base font-bold text-slate-900 mb-1">No Scheduled Posts</h3>
                <p className="text-xs text-slate-600">Posts will appear here when scheduled.</p>
              </Card>
            ) : (
          ...
          <TabsContent value="posted" className="space-y-2 mt-3">
            {postedContent.length === 0 ? (
              <Card className="bg-white border border-slate-200 rounded-lg p-6 text-center shadow-sm">
                ...
                <h3 className="text-base font-bold text-slate-900 mb-1">No Posted Content</h3>
                <p className="text-xs text-slate-600">Published posts will appear here.</p>
              </Card>
            ) : (
